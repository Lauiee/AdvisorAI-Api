...
요구사항:
1. 리포트 형식으로 작성하며, 반드시 다음 소목차들을 순서대로 사용해야 합니다:
   - ## 요약 (100-150자): 전체 매칭 결과를 간략히 요약
   - ## 1차 적합도 분석 (200-250자): 1차 적합도 점수와 각 세부 지표를 상세히 분석
   - ## 채팅 기반 분석 (200-250자): 채팅 내용을 바탕으로 한 심층 분석
   - ## 최종 평가 및 결론 (200-250자): 1차 적합도와 채팅 분석을 종합한 최종 평가
   - ## 추천 사항 (150-200자): 구체적인 강점, 개선점, 추천 사항 제시
2. 1차 적합도와 채팅 기반 분석을 종합하여 평가
3. 채팅 분석 내용을 반드시 리포트에 포함시켜야 합니다
4. 구체적인 강점과 개선점 제시
5. 전문적이고 객관적인 문체
6. 각 항목별로 위에 명시된 분량을 충족해야 하며, 전체 분량은 약 850-1100자 정도가 되어야 합니다
7. 각 문단 사이에는 반드시 줄바꿈(빈 줄)을 넣어주세요
8. 중요: 리포트는 반드시 "# 최종 매칭 리포트"로 시작해야 합니다
9. 중요: 소목차는 반드시 위에 명시된 순서와 정확한 이름으로 "## 소목차명" 형식의 마크다운을 사용해야 합니다
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "당신은 대학원 진학 상담 전문가입니다. 지원자와 교수님의 매칭을 종합적으로 분석하여 전문적인 리포트를 작성합니다. 리포트는 반드시 \"# 최종 매칭 리포트\"로 시작해야 합니다. 소목차는 반드시 다음 순서와 정확한 이름으로 작성해야 합니다: \"## 요약\", \"## 1차 적합도 분석\", \"## 채팅 기반 분석\", \"## 최종 평가 및 결론\", \"## 추천 사항\". 다른 제목이나 형식을 사용하지 마세요."
                },
                {
                    "role": "user",
    ...
    try:
        ...
        stream = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "당신은 대학원 진학 상담 전문가입니다. 지원자와 교수님의 매칭을 종합적으로 분석하여 전문적인 리포트를 작성합니다. 각 문단 사이에는 반드시 줄바꿈(\\n)을 넣어 구조화된 형태로 작성하세요. 리포트는 반드시 \"# 최종 매칭 리포트\"로 시작해야 합니다. 소목차는 반드시 다음 순서와 정확한 이름으로 작성해야 합니다: \"## 요약\", \"## 1차 적합도 분석\", \"## 채팅 기반 분석\", \"## 최종 평가 및 결론\", \"## 추천 사항\". 다른 제목이나 형식을 사용하지 마세요."
                },
                {
                    "role": "user",
                    "content": report_prompt
...
10. 중요: 이메일 마지막에 지원자 이메일 주소나 전화번호를 절대 포함하지 마세요. 서명은 이름만으로 끝내세요.
...
중요: 이메일 마지막에 지원자의 이메일 주소나 전화번호를 절대 포함하지 마세요. 서명은 "[이름] 드림" 형식으로 끝내세요.
