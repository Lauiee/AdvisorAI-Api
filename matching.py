...
10. 중요: 소목차(요약, 상세 분석, 결론 등)는 반드시 "## 소목차명" 형식의 마크다운을 사용해야 합니다
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "당신은 대학원 진학 상담 전문가입니다. 지원자와 교수님의 매칭을 종합적으로 분석하여 전문적인 리포트를 작성합니다. 리포트는 반드시 \"# 최종 매칭 리포트\"로 시작해야 합니다. 소목차(요약, 상세 분석, 결론 등)는 반드시 \"## 소목차명\" 형식의 마크다운을 사용해야 합니다. 다른 제목이나 형식을 사용하지 마세요."
                },
                {
                    "role": "user",
    ...
    try:
        ...
        stream = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "당신은 대학원 진학 상담 전문가입니다. 지원자와 교수님의 매칭을 종합적으로 분석하여 전문적인 리포트를 작성합니다. 각 문단 사이에는 반드시 줄바꿈(\\n)을 넣어 구조화된 형태로 작성하세요. 리포트는 반드시 \"# 최종 매칭 리포트\"로 시작해야 합니다. 소목차(요약, 상세 분석, 결론 등)는 반드시 \"## 소목차명\" 형식의 마크다운을 사용해야 합니다. 다른 제목이나 형식을 사용하지 마세요."
                },
                {
                    "role": "user",
                    "content": report_prompt
